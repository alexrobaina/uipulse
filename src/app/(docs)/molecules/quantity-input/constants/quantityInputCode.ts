export const quantityInputCode = `import React, { useState, useEffect } from "react";
...
const QuantityInput: React.FC<QuantityInputProps> = ({
  ...
  return (
    <div className={cn("flex items-center", className)}>
      ...
      >
        <Minus className={cn("text-neutral-600 dark:text-neutral-400", sizeClasses[size].icon)} />
      </button>
      ...
      >
        <Plus className={cn("text-neutral-600 dark:text-neutral-400", sizeClasses[size].icon)} />
      </button>
    </div>
  );
};
